...
export class RuntimeMetrics {
  ...
  constructor(registry: Registry) {
    ...
    this.external = registry.gauge('nodejs.external', this.withVersion());
    this.heapTotal = registry.gauge('nodejs.heapTotal', this.withVersion());
    this.heapUsed = registry.gauge('nodejs.heapUsed', this.withVersion());
    this.rss = registry.gauge('nodejs.rss', this.withVersion());

    this.eventLoopActive = registry.gauge('nodejs.eventLoopUtilization', this.withVersion());
    this.eventLoopLagTimer = registry.timer('nodejs.eventLoopLag', this.withVersion());
    this.eventLoopTime = registry.timer('nodejs.eventLoop', this.withVersion());

    this.allocationRate = registry.counter('nodejs.gc.allocationRate', this.withVersion());
    this.liveDataSize = registry.gauge('nodejs.gc.liveDataSize', this.withVersion());
    this.maxDataSize = registry.gauge('nodejs.gc.maxDataSize', this.withVersion());
    this.promotionRate = registry.counter('nodejs.gc.promotionRate', this.withVersion());

    this.openFd = registry.gauge('openFileDescriptorsCount', this.withVersion());
    this.maxFd = registry.gauge('maxFileDescriptorsCount', this.withVersion());

    this.cpuUsageSystem = registry.gauge('nodejs.cpuUsage', this.withVersion({'id': 'system'}));
    this.cpuUsageUser = registry.gauge('nodejs.cpuUsage', this.withVersion({'id': 'user'}));
  }
  ...
  measureGcEvents(emitGcFunction: EmitGcFunction): void {
    // this function becomes a call back in EmitGCEvents, so save a reference to the current object
